                  ...
                  <div className="flex flex-wrap justify-center gap-6">
                    {members.map((member, index) => (
                      <div
                        ...
                        className="w-[250px] sm:w-[220px] bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl overflow-hidden hover:bg-[#1a1a1a]/90 hover:border-[#a02638]/50 transition-all duration-300 hover:scale-105 group text-left shadow-lg hover:shadow-xl cursor-pointer"
                      >
                          ...
                          <div className="relative group overflow-hidden rounded-t-xl">
                            ...
                            {member.social?.instagram && (
                              <div
                                onClick={(e) => {
                                  e.stopPropagation();
                                  const handle = member.social.instagram.trim();
                                  const url = handle.startsWith('http') 
                                    ? handle 
                                    : `https://instagram.com/${handle.replace(/^@/, '')}`;
                                  window.open(url, '_blank', 'noopener,noreferrer');
                                }}
                                className="absolute top-3 right-3 p-1.5 bg-black/60 hover:bg-[#a02638] text-white rounded-full transition-colors duration-300 z-10 cursor-pointer flex items-center justify-center backdrop-blur-sm"
                                title="Instagram"
                              >
                                <svg
                                  className="w-5 h-5"
